...
const index = ({
  ...
  const handleRequest = () => {
    if (mounted) {
      setsubmitting(true);
      axios
        .get(`/api/checkout/poof/topup/${topupId}/`)
        .then((data) => {
          setsubmitting(false);
          setPaymentinfo(() => {
  ...
  const orderInfo = () => {
    return (
      <>
        ...
        >
          <h1 className="text-white font-bold text-2xl my-2">Topup details</h1>
          <div>
          ...
          <div>
            ...
            <p className="mb-3">{currencyLogo?.coin}</p>
          </div>
          <div>
            <h1 className={"font-bold text-[#99a29e]"}>Topup id</h1>
            <p className="mb-4">{topupId}</p>
          </div>
        </div>
      </>
    );
  ...
  useEffect(() => {
    ...
    intervalId = setInterval(() => {
      if (
        (paymentinfo && !paymentinfo?.paid && !isDelete) ||
  ...
  const Loader = (variant: any, width?: number, height?: number) => {
    return (
      <Skeleton
        variant={variant}
        width={width}
                              ...
                              <button
                                onClick={async (e) => {
                                  ...
                                  if (r.isConfirmed) {
                                    axios
                                      .delete(
                                        `/api/orders/delete_topup_order/${topupId}/`
                                      )
                                      .then((res) => {
                                        if (res.data.success) {
                                          setIsDelete(false);
                            ...
                            {paymentinfo.paid ? (
                              <div className="flex flex-col gap-5 h-[100%] w-[80%] mx-auto justify-center items-center text-white">
                                ...
                                paymentinfo.paymentPaid !== "PENDING" ? (
                                  <>
                                    <div className="text-center h-[60%] mt-4 text-white justify-center flex flex-col items-center">
                                      ...
                                      <p className="text-white text-sm mt-2 mb-10 w-[70%]">
                                        ...
                                        {paymentinfo?.coin} and are waiting for
                                        it to be confirmed on the network. You
                                        may continue{" "}
                                        <a
                                  ...
                                  <>
                                    <Success orderId={topupId} topup={true} />
                                  </>
                                )}
                              </div>
                            ) : (
